...
const SERVICES = [
  {
    icon: "📸",
    label: "Photography",
    desc: "Professional photographers & studios for weddings, portraits & events",
    href: "/photography",
    active: true,
  },
  {
    icon: "🎨",
    label: "Decorators",
    desc: "Floral, lighting, theme & stage decoration for every occasion",
    href: "#",
    active: false,
  },
  {
    ...
    desc: "Bridal, editorial & event makeup and hair styling specialists",
    href: "#",
    active: false,
  },
  {
    icon: "🏛️",
    label: "Venues",
    desc: "Banquet halls, gardens, rooftops & unique event spaces",
    href: "#",
    active: false,
  },
  {
    icon: "🎵",
    label: "Entertainment",
    desc: "DJs, live bands & performers to keep your guests engaged",
    href: "#",
    active: false,
  },
  {
    icon: "🍽️",
    label: "Catering",
    desc: "Full-service food & beverage for events of all sizes",
    href: "#",
    active: false,
  },
];
...
const PlanMyEvent = () => {
  ...
  return (
    ...
    >
      ...
      {/* Content */}
      <div className="relative flex flex-col items-center px-6 pt-16 pb-12" style={{ marginTop: '-20px' }}>
        ...
        <div className="text-center mb-8 mt-6">
          ...
          >
            Browse our verified event professionals by category.
          </p>
        </div>
        ...
        <div className="w-full max-w-[420px] flex flex-col gap-3">
          {SERVICES.map((s) => (
            <button
              key={s.label}
              onClick={() => s.active && navigate(s.href)}
              disabled={!s.active}
              className="group w-full flex items-center gap-4 rounded-2xl px-5 py-4 text-left transition-all active:scale-[0.98]"
              style={{
                background: s.active ? "rgba(255,255,255,0.88)" : "rgba(255,255,255,0.5)",
                border: "1px solid hsl(6 28% 89%)",
                boxShadow: s.active ? "0 2px 16px hsl(6 28% 50% / 0.06)" : "none",
